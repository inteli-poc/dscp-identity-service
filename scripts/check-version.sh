...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/vitalam-identity-service/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/vitalam-identity-service/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/vitalam-identity-service/Chart.yaml)
